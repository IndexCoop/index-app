...
export const RaffleBanner = () => {
  ...
  const timeLeft = useEpochCountdown(epoch?.endDate)
  const hasActiveEpoch = !!epoch && !epoch.silent
  ...
  return (
    <div className='relative flex w-full items-center gap-3 rounded-lg border border-[#496C72] bg-[#1A2B2F] px-4 py-3 sm:gap-6'>
      ...
      {hasActiveEpoch && timeLeft && (
        <p className='text-ic-blue-300 hidden text-xs font-semibold sm:block'>
          Ends in {timeLeft}
        </p>
      )}
      <Button
